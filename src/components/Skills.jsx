          ...
          {skillCategories.map((category, i) => (
            ...
            >
              <div className="flex items-center mb-6 gap-3">
                {category.icon}
                <h3 className="text-2xl sm:text-3xl font-semibold text-gray-900">{category.title}</h3>
              </div>
              <div className="space-y-4">
                {category.skills.map((skill, idx) => (
                  <motion.div key={idx} variants={fadeUp}>
                    <div className="flex justify-between text-gray-800 font-medium mb-1 text-sm sm:text-base">
        ...
        >
          {certifications.map((cert, i) => (
            ...
            >
              ...
              <h4 className="text-2xl sm:text-3xl font-semibold text-gray-900 mb-2">{cert.title}</h4>
              <p className="text-lg sm:text-xl text-gray-600 mb-4">{cert.organization}</p>

              <div className="flex justify-center items-center gap-3 mb-4">
                <span className={`px-4 py-1 rounded-full text-sm sm:text-base font-medium ${cert.color}`}>{cert.level}</span>
